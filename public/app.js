...
function search() {
  ...
  let filteredData = data.filter((exchange) =>
...
function filterByPrice() {
  ...
  let minInputValue = document.querySelector("#min-price").value;
  let maxInputValue = document.querySelector("#max-price").value;
  //cast to number
...
function filterByVolume() {
  ...
  //filter the data according to the prices
  let filteredData = data.filter(
    (exchange) => exchange.volume >= minVolume && exchange.volume <= maxVolume
  );
  //display data
  updatePage(filteredData);
}
...
function sortData() {
  ...
  const selectSortBy = document.querySelector("#sort-by").value;
  switch (selectSortBy) {
    case "volume":
